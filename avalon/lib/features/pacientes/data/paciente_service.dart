...
class PacienteService {
  ...
  Future<List<Paciente>> getAll() async {
    ...
    try {
      final res =
          await _client.rpc('get_pacientes', params: {'p_token': _token});
      final pacientes = (res as List)
          .map((j) => Paciente.fromJson(j as Map<String, dynamic>))
          .toList();
      AppLogger.database('Pacientes obtenidos: ${pacientes.length}');
      return pacientes;
    } catch (e, stackTrace) {
      AppLogger.database('Error obteniendo pacientes: $e',
  ...
  Future<List<Paciente>> buscar(String query) async {
    ...
    try {
      ...
      AppLogger.database('Error en búsqueda de pacientes: $e',
          error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Paciente> crear({
    ...
    try {
      ...
      if (historialMedico != null) {
        params['p_historial_medico'] = historialMedico;
      }
      if (objetivosTerapeuticos != null) {
        params['p_objetivos_terapeuticos'] = objetivosTerapeuticos;
      }
      if (fechaNacimiento != null) {
        params['p_fecha_nacimiento'] =
      ...
      AppLogger.database('Error creando paciente: $e',
          error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> eliminar(String id) async {
    ...
    try {
      await _client
          .rpc('eliminar_paciente', params: {'p_token': _token, 'p_id': id});
      AppLogger.database('Paciente eliminado exitosamente');
    } catch (e, stackTrace) {
      AppLogger.database('Error eliminando paciente: $e',
          error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> toggleActivo(String id, bool activo) async {
    ...
    try {
      ...
      AppLogger.database('Error actualizando estado del paciente: $e',
          error: e, stackTrace: stackTrace);
      rethrow;
    }
  }

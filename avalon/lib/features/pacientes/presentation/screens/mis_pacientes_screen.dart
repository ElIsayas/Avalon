...
import '../providers/paciente_provider.dart' as paciente_providers;
import '../../shared/widgets/desktop_layout2.dart';

class MisPacientesScreen extends ConsumerStatefulWidget {
  const MisPacientesScreen({super.key});

  @override
  ConsumerState<MisPacientesScreen> createState() => _MisPacientesScreenState();
}

class _MisPacientesScreenState extends ConsumerState<MisPacientesScreen> {
  ...
  Future<void> _refreshPacientes() async {
    final notifier = ref.read(paciente_providers.pacienteProvider.notifier);
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(paciente_providers.pacienteProvider);
  ...
  void _filterPacientes() {
    ...
    switch (_selectedFilter) {
      ...
      case 'todos':
      default:
        // Ya están todos los pacientes filtrados por búsqueda
        break;
    }
    
    // Actualizar estado con pacientes filtrados
    final notifier = ref.read(paciente_providers.pacienteProvider.notifier);
